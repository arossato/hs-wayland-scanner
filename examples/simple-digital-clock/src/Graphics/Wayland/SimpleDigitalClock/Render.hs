...
import Graphics.Wayland.SimpleDigitalClock.Cairo
...
updatePoolText p (dateLine, timeLine) = do
  let buf = poolBuffer p
      raw = castPtr (bufferPtr buf) :: Ptr Word8
      w   = fromIntegral (bufferWidth  p) :: CInt
      h   = fromIntegral (bufferHeight p) :: CInt
      s   = fromIntegral (bufferStride p) :: CInt
  renderWithRaw raw 0 w h s $ do
    -- Background
